import React from "react";
import { Row, Col, Container } from "react-bootstrap";
import Product from "../components/Product";
import products from "../products";

function ClothesScreen() {
  const fontStyle = {
    fontFamily: "'Montserrat', sans-serif",
  };
  const clothes = products.filter((item) => item.category === "clothes");

  return (
    <div style={fontStyle}>
      {/* Hero Banner */}
      <div
        style={{
          background: "linear-gradient(135deg, #AEC6FF, #FFE6F7)",
          padding: "60px 0",
          textAlign: "center",
          color: "#333",
        }}
      >
        <h1 style={{ fontWeight: "700", fontSize: "2.5rem" }}>Clothing Hub</h1>
        <p style={{ marginTop: "10px", fontSize: "1.1rem", fontWeight: "300" }}>
          Fresh styles. Fresh looks. Explore what fits your vibe.
        </p>
      </div>

      {/* Products */}
      <Container className="py-5">
        <Row className="gy-4">
          {clothes.map((product) => (
            <Col key={product._id} xs={12} sm={6} md={4} lg={3}>
              <Product product={product} />
            </Col>
          ))}
        </Row>
      </Container>
    </div>
  );
}

export default ClothesScreen;
